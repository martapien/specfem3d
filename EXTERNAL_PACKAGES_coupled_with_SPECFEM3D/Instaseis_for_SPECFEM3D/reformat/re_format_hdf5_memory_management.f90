program re_format_hdf5
  ...
  ! Iterators
  integer itime, irec, irec_min, irec_max, irec_loc

  ! helpers to iterate over 8mb chunks
  integer s, s_mod, s_index, factor, offset_chunk
  ...
  factor = int((8 * 1024 * 1024 / 9) / ntime)
  s = int((nrec_by_proc(myrank+1) / float(factor)) + 0.5) ! nb of 8mb blocks
  s_mod = mod(nrec_by_proc(myrank+1), float(factor))
  ...
  do s_index = 0, s-1
    ...
    allocate(velocity(3, ntime, factor))
    allocate(stress(6, ntime, factor))
    v_dims(1) = 3
    v_dims(2) = ntime
    v_dims(3) = factor
    ...
    offset_chunk = int(offset_by_proc(myrank+1) + s_index * factor)
    offsetf  = (/0, 0, offset_chunk/)
    if (s_index == s-1) then
        v_countf = (/3, ntime, factor/)
        s_countf = (/6, ntime, factor/)
    endif
    if (s_index /= s-1) then
        v_countf = (/3, ntime, s_mod/)
        s_countf = (/6, ntime, s_mod/)
    endif
    ...
    (vr(3, nrec_by_proc(myrank+1)), tr(3, nrec_by_proc(myrank+1)))
    int(offset_by_proc(myrank+1) + s_index * factor)

    irec_min = s_index * factor

    if (s_index == s-1) then
        irec_max = s_index * factor + s_mod
    endif
    if (s_index /= s-1) then
        irec_max = s_index * factor + factor
    endif
    do itime = 1, ntime
     do irec = irec_min, irec_max
        irec_loc = irec - irec_min
        vr(1, irec) = velocity(1, itime, irec_loc)
        vr(2, irec) = velocity(2, itime, irec_loc)
        vr(3, irec) = velocity(3, itime, irec_loc)

        tmp1 = dble(stress(1, itime, irec_loc)) * dble(normal(1, irec)) &
           + dble(stress(6, itime, irec_loc)) * dble(normal(2, irec)) &
           + dble(stress(5, itime, irec_loc)) * dble(normal(3, irec))
        tr(1, irec) = real(tmp1)
        tmp2 = dble(stress(6, itime, irec_loc)) * dble(normal(1, irec)) &
           + dble(stress(2, itime, irec_loc)) * dble(normal(2, irec)) &
           + dble(stress(4, itime, irec_loc)) * dble(normal(3, irec))
        tr(2, irec) = real(tmp2)
        tmp3 = dble(stress(5, itime, irec_loc)) * dble(normal(1, irec)) &
           + dble(stress(4, itime, irec_loc)) * dble(normal(2, irec)) &
           + dble(stress(3, itime, irec_loc)) * dble(normal(3, irec))
        tr(3, irec) = real(tmp3)
