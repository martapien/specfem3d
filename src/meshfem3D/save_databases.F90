...
!---------------------------------------------------------------------------------------------------------------
  ...
  integer   :: nlayer  !! number of layers in the 1D model
...
!! VM VM add files in case of AxiSEM coupling
  if ((INJECTION_TECHNIQUE_TYPE == INJECTION_TECHNIQUE_IS_AXISEM) .or. &
...
1000 format(3f30.10)
    ...
    ! MPC instead of reformatting DSM into a drifferent file
    ! we already provide the right file so here we just read it off
       ! read 1D model  -------------------------------------------
